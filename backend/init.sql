-- Database: document_management_db (or your preferred database name)

-- Drop tables if they exist, in an order that respects foreign key constraints.
-- Starting with tables that don't have foreign keys or have foreign keys referencing other tables that will be dropped later.
...
CREATE TABLE `user_vocabulary` (
                                   ...
                                   `user_id` INT(11) NOT NULL,
                                   `word_id` INT(11) NOT NULL,
                                   `word` VARCHAR(200) NOT NULL, -- Added for faster lookup and direct access
                                   `language` VARCHAR(20) NOT NULL DEFAULT 'en',
                                   `definition` TEXT,
                                   `example` TEXT,
                                   `notes` TEXT,
                                   `status` VARCHAR(50) DEFAULT 'new', -- e.g., 'new', 'learning' (reviewing), 'mastered' (known)
                                   ...
                                   UNIQUE KEY `unique_user_word` (`user_id`, `word`, `language`), -- Unique constraint on user, word, and language
...
CREATE TABLE `review_items` (
                                ...
                                `review_type` VARCHAR(50) DEFAULT NULL, -- e.g., 'flashcard', 'quiz'
...
CREATE TABLE `daily_learning_stats` (
                                        ...
                                        `total_study_time` INT(11) DEFAULT 0, -- in seconds
                                        `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
                                        `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                                        PRIMARY KEY (`stat_id`),
                                        UNIQUE KEY `unique_user_date` (`user_id`, `learning_date`),
                                        KEY `user_id` (`user_id`),
...
CREATE TABLE IF NOT EXISTS `vocabulary_mastery_stats` (
...
CREATE TABLE IF NOT EXISTS `spaced_repetition_schedule` (
                                                            ...
                                                            `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                                                            FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE,
                                                            FOREIGN KEY (`user_vocab_id`) REFERENCES `user_vocabulary` (`user_vocab_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `note_attachments` (
                                                  ...
                                                  `file_size` BIGINT,
                                                  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                                                  FOREIGN KEY (`note_id`) REFERENCES `document_notes` (`note_id`) ON DELETE CASCADE,
                                                  FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Add necessary indexes for performance on vocabulary tables
CREATE INDEX IF NOT EXISTS idx_user_vocabulary_user_id ON user_vocabulary (user_id);
CREATE INDEX IF NOT EXISTS idx_user_vocabulary_language ON user_vocabulary (language);
CREATE INDEX IF NOT EXISTS idx_user_vocabulary_status ON user_vocabulary (status);
CREATE INDEX IF NOT EXISTS idx_user_vocabulary_mastery_level ON user_vocabulary (mastery_level);
CREATE INDEX IF NOT EXISTS idx_user_vocabulary_next_review_at ON user_vocabulary (next_review_at);

CREATE INDEX IF NOT EXISTS idx_user_vocabulary_tags_user_vocab_id ON user_vocabulary_tags (user_vocab_id);
CREATE INDEX IF NOT EXISTS idx_user_vocabulary_tags_tag_id ON user_vocabulary_tags (tag_id);

CREATE INDEX IF NOT EXISTS idx_vocabulary_tags_tag_name ON vocabulary_tags (tag_name);

CREATE INDEX IF NOT EXISTS idx_review_items_session_id ON review_items (session_id);
CREATE INDEX IF NOT EXISTS idx_review_items_user_vocab_id ON review_items (user_vocab_id);
CREATE INDEX IF NOT EXISTS idx_review_items_word_id ON review_items (word_id);
CREATE INDEX IF NOT EXISTS idx_review_items_review_date ON review_items (review_date); -- Assuming review_date column exists in review_items

CREATE INDEX IF NOT EXISTS idx_daily_learning_stats_user_id ON daily_learning_stats (user_id);
CREATE INDEX IF NOT EXISTS idx_daily_learning_stats_learning_date ON daily_learning_stats (learning_date);
